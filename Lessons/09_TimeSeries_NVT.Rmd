...
* Can we **forecast** conditions in the future? or **Backcast**
...
Neuse_ts <- ts(NeuseFlow[[4]], frequency = 365) #double brackets for ts function
#time series data stored in values, can view in data
...
#seasonal - same over time
#trend- moving average
#remainder - error or random component- everything thats left
#bar on right show difference in scale
...
ggplot(Neuse_Components) +
  ...
  labs(x = "", y = "Discharge (cfs)")
#moving average is smoothing out the variability we see in the data set
#trend is always above 0, you would think that there would be some negative trend at some point
...
ggplot(Neuse_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  labs(x = "", y = "Discharge (cfs)")
#seasonal shows a negative trend - allowing the trend to always be posistive. There isnt such things as a negative discharge but it captures the seasonal trend. 
...
* **Piecewise constant**: AKA nearest neighbor
* **Linear**: connnect the dots
* **Spline**: quadratic
...
Months <- data.frame(Date_monthrounded = seq.Date(from = as.Date("1980-10-01"), to = as.Date("2021-12-01"), by = "month")) #gives the first of each month
...
linearinterpolation <- as.data.frame(approx(NeuseCond_monthly$Conductivity, n = 566, method = "linear")) #approx does interpolations, linear or piecewise (does not do spline)
NeuseCond_monthly$Conductivity <- linearinterpolation$y #replace conductivity columns with y columns from linear interp
...
```{r}
NeuseCond_monthly_ts <- ts(NeuseCond_monthly[[3]], frequency = 12) #double brackets for ts function
#time series data stored in values, can view in data
