...
-Higher variability in flow, flashier
-shifts in peak discharge, snowmelt melting sooner
-snow coming as rain
-more evapotranspirtation
stationarity
...
$T = (n+1)/m$ #$ around text turns it into an equation - makes it appear as an equation

T=recurrance interval
n=years
m=ranking
probability of something of that ranking happening in a given year for more
...
stationarity = no trend over time
...
EnoDischarge <- readNWISdv(siteNumbers = "02085070",
                     ...
                     startDate = "", 
                     endDate = "2021-09-30") #water year for USGS, year begins october 1. 
...
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  labs(x = "Year", y = "Discharge (cfs)") # not a lot of seasonality
```
...
EnoDischarge <- EnoDischarge %>%
  ...
  filter(WaterYear != "1963") #factor

# Water Year is a factor. We want to re-classify as numeric.
EnoDischarge$WaterYear <- as.numeric(as.character(EnoDischarge$WaterYear)) #need to tell R is a character, then a number

EnoRecurrence <- 
  EnoDischarge %>%
  filter(WaterYear < 1994) %>%
  group_by(WaterYear) %>%
  summarise(PeakDischarge = max(Discharge)) %>% #max of discharge for any one given year
  mutate(Rank = rank(-PeakDischarge), # reverse ranking using (-) give number 30 to lowest value
         RecurrenceInterval = (length(WaterYear) + 1)/Rank, 
         Probability = 1/RecurrenceInterval) #any given year has this probabilty that you will see this amount of disharge or more every number of recurrance years
...
ggplot(EnoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  scale_x_log10() + #makes relationship more linear - classic log10 transformation
  geom_smooth(method = "lm", color = "black", se = FALSE) #se = flase gets ride of cone around line

Eno.RImodel <- lm(data = EnoRecurrence, PeakDischarge ~ log10(RecurrenceInterval))
summary(Eno.RImodel)

#r squared- 91 is pretty high -over 91% of variability is explained by peak discharge
...
#this spits out discharge estimates for 100 and 500 year floods
#with climate change, recurrance interval methodology is not as reliable
```
...
ggplot(EnoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  ...
  scale_x_log10()
# the recurrance intervals goes up, we see a difference in discharge

Eno.RImodel.Late <- lm(data = EnoRecurrence.Late, PeakDischarge ~ log10(RecurrenceInterval))
summary(Eno.RImodel.Late)
Eno.RImodel.Late$coefficients #big increase in slope in the later years
...
#100 and 500 year flood estiamtes are much higher than before
...
Is the eno displaying stationary behvaior over time? NO
we cannot assume stationarity

Forecasting - predicting discharge - conclusions would be very different depending on which time period of data you use
...
#Qualitative analysis: super peaky, but not regular - event based system. not occuring on common season scale. Peaks are getting larger.can be a result of urbanization and channelization. 
...
ggplot(GreenDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  labs(x = "Year", y = "Discharge (cfs)")
#we can see a drop in peak discharge after 1961. Dam successful in flood control. can realease water over a greater period of time to manage flooding
```

how to tease out climate change vs urbanization
