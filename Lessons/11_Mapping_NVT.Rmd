...
states #the data for this has the polygon points that make up each state
...
## Data wrangling
```{r}
help.search("datasets", package = "LAGOSNE") #gives more information about the data set
...
LAGOScombined <- 
  ...
  left_join(., LAGOSlimno) %>% # "." means use whatever you created in the line before
  left_join(., LAGOSstate) %>%
  filter(!is.na(state)) %>%
  select(lagoslakeid, sampledate, secchi, lake_area_ha, maxdepth, nhd_lat, nhd_long, state)
#because its a left join you should have as many lines as lagosnutrient as its the first dataset, minus a few for drop_na
```
...
```{r}
secchi.summary.mean <- LAGOScombined %>%
...
```{r}
secchi.spatial.mnean <- st_as_sf(secchi.summary.mean, coords = c("long", "lat"), crs = 4326)
secchi.spatial.min <- st_as_sf(secchi.summary.min, coords = c("long", "lat"), crs = 4326)


ggplot() +
  geom_sf(data = states.subset, fill = "white") +
  geom_sf(data = secchi.spatial.mean, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
...
ggplot() +
  geom_sf(data = states.subset, fill = "white") +
  geom_sf(data = secchi.spatial.min, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) + #scale met brewer
  labs(color = "Min Secchi Depth (m)") +
  theme(legend.position = "top")
