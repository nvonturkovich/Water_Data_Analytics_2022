...
> aggregation: taking a certain time step and moving up to a higher time step, daily to weekly, weekly to monthly. Reason: some time series require no autocorrelation, (daily data being too similar) so you need to aggregate to monthly. 

> subsampling: opposite direction - zooming in instead of zooming out. sub sample - some data to build model, some data to test model. 
Taking a subsample of data (every 14th day to avoid autocorrelation)
...
NeuseCond <- readWQPqw(siteNumbers = "USGS-02089500", # Neuse River at Kinston, NC
                     ...
                     endDate = "2021-12-31")#man kendall needs a start and end date
...
ggplot(NeuseFlow_monthly, aes(x = as.factor(Month), y = Discharge_acftmo)) +
  geom_boxplot()+
  labs(x="Month", y = "Discharge (acre*ft/month)")
...
z=score = positive = spacing is positive?
larger your z score, smaller your P
S - tracks trends over time
this example is telling us we have a positive trend in conductivity over time. Conductivty is proxy for salinity. Water getting more salty over time. 
looking at summary:
most significant increases in winter months. Baseline getting higher rather than peaks getting higher
>
...
aggregated data - sum of monthly data - need to interpret differently
```{r}
...
What would we conclude based on these findings? 
smaller z score than conductivity, positive, increasing over time
increase dishcharge in low flow months
P value = is there a trend
S score = directionality  and strength of trend
...
anticipating that measurements are correlated over time
...
covariance(variability over time) between 
...
```
p<.05 This is telling us that the data is not changing over time
...
acf(NeuseFlow_timeseries) #if we give one month of data, how correlated will it be
pacf(NeuseFlow_timeseries)

```
ACF = autocorrelation
autocrrelation decays over time, and we see distinct seasonality

PCAF
decays alot faster, we do not see as much seasonablity
...
```
why is dotted line so bad?
range of random variability is so largem that even though 
