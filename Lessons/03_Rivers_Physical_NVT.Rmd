...
morphology-shape of stream, cross sectionally and over distance
channel-senuosity
sediment type-friction
discharge- what volume of water is moving throug the stream
grade- slope of stream bed
...
*rating curve gives you the discharge based on depth of river

```{r}
EnoDischarge.ratingcurve <- readNWISdv(siteNumbers = "02096500",
                     parameterCd = c("00060", "00065"), #60 is discharge, 65 is Gage height
...
-as gage height increases discharge increases
...
check out the data retrieval package - lots of capability here!
...
parameter code differs - 

approval code A - means data is legit
...
BitterrootDischarge.Historical <- readNWISdv(siteNumbers = "12344000",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "") #blank will grade all the record available
...
BitterrootDischarge.Pattern <- BitterrootDischarge.Historical %>%
  ...
  summarise(Median.Discharge = median(Discharge, na.rm = TRUE), 
            p95.Discharge = quantile(Discharge, 0.95, na.rm = TRUE), #quantile function- can specify anything between 0-1. need to remove NAs
            p5.Discharge = quantile(Discharge, 0.05, na.rm = TRUE))

ggplot(BitterrootDischarge.Pattern, aes(x = DOY)) +
  geom_line(aes(y = Median.Discharge)) + #each as an indivdual geom line with a different color
...
showing seasonality by using day of year as opposed to month?
day of year works well 
...
Just Bitterroot graph
```{r}
BitterrootPlot <- 
  ggplot(CombinedDischarge, aes(x = Date, y = Bitterroot.Discharge)) +
  geom_line() +
  labs(x = "", y = expression("Discharge (ft"^3*"/s)"), title = "Bitterroot River") + 
  theme(plot.title = element_text(margin = margin(b = -10), size = 12), axis.title.x = element_blank())
BitterrootPlot
```

nehalem
western cascades. winter peaks, effected by developement
