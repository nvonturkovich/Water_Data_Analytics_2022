...
#rm() will clear specific data sets
...
Linear regression to quantity relationship between stressors and responders
...
#NA state. drop_na on state column is a better function. removes na's from one column 
...
LAGOSMN<-LAGOSjoined %>% 
  ...
  select(c(lagoslakeid, sampledate, state_name, hu4, chla, tn, tp)) %>% 
  mutate(year = year(sampledate), month = month(sampledate)) %>% 
  filter(state_name == "Minnesota") %>% 
  filter(month %in% c(6:9)) %>% 
  filter(hu4 %in% c("0401", "0701", "0702", "0703", "0704", "0902"))

ggplot(LAGOSMN, aes(x = tp, y = chla)) + #or "color=hu4" instead of facet wrap
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(vars(hu4)) +
  scale_y_log10() + #log10 often used in this relationship
  scale_x_log10() 
  #labs(x = expression("TP ("*mu*"g/L)"), y = expression("Chl a ("*mu*"g/L"))

MNmodel<-lm(data= LAGOSMN, log10(chla) ~log10(tp)*hu4) #interaction with hu4
#low p value means we are seeing 
summary(MNmodel)
par(mfrow = c(2,2))
plot(MNmodel)
#look for all the assumption violations here
